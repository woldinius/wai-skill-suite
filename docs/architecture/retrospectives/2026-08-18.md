...
2. **Decide the ledger's home** (the human) — issue #35's two honest options: move it to
   ...
   *Note (2026-09-13):* decided in #35 as the second option — in-repo, and the gate said
   *"belongs on main"* on every branch run. Revised in #66: a row rides the PR that produced it,
   and the note fires only when a branch has no open PR; `LEDGER_HOME=main` keeps the collection
   path. Both measurements: `docs/rationale/merge-gate.md` § *Where a row lives*.
