...
| Q7 | Does the suite work outside its author's repos? | One author; eight field runs; the largest dataset comes from a repo of a different product shape — a game ([field report](field-reports/2026-09-06-two-months-of-gate-259-verdicts.md), dated) — but it is still the author's own, so the question stays open | ≥ 3 foreign repos with a pasted `gate-stats.sh` extract — see the field-report template |
