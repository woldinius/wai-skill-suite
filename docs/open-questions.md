...
|---|---|---|---|
| Q1 | Does the gate ever say GO in this repo — or is "fail-closed" where it has always been? | **17 gate verdicts on record, 3 GO rows** (PR #17, 2026-08-06; PR #1 and PR #4, 2026-08-11), **10 untagged**. A field repo reports 30 runs with GO 5 · NO-GO 22 · UNKNOWN 0 ([field report](field-reports/2026-08-06-thirty-runs-zero-false-negatives.md), dated) | Answered for existence on 2026-08-06 — here and in a second repo; stays open as a rate until the ledger has enough GO rows to review weekly |
| Q2 | Does the model actually *run* the gate, or check from memory? (Test 0) | 1 documented field run (empirics, Run 1) + this repo's ledger; **bypass rate unmeasured** | ≥ 30 review runs; report the share with a matching ledger row. Below ~90 %, the determinism claim fails at the execution layer — and that gets published too |
| Q3 | How often is the gate wrong? | This repo: 5 judged NO-GOs, 0 fp; the GO row tagged `ok`; one UNKNOWN row held as "need inspection"; fn = 0 — n still small. A field repo reports 12 judged NO-GOs with 0 fp and 0 fn over 30 runs ([field report](field-reports/2026-08-06-thirty-runs-zero-false-negatives.md), dated — its numbers age with it) | ≥ 20 human-tagged rows here; read with `gate-stats.sh`. One `fn` outweighs ten `fp` |
