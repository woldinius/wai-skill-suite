...
| **Scripts a skill invokes** | `merge-gate.sh`, `catalog-lint.sh`, the audit and protocol lints — 24 scripts across the suite | In principle yes — which is why this was **measured**, not assumed ([Test 0](docs/empirics.md)) |
...
[`tests/`](tests/) exists — 227 cases, **founded** on bugs that shipped and grown into the
...
- **Installs/updates** the suite's skills into `.claude/skills/` — and **nothing else**. It
  ...
  catalog over would hand you a 91-ID multi-surface document with the setup questions
...
tests/                                           # 227 cases for the deciding scripts — founded on bugs that shipped
